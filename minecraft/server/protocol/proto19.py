...
# CHANGES FROM PROTO5:
#
# Shorts in packets 1 in and 1 out now use varints
#
#
#
...
def handle_login(server, conn, addr, handshake, data_buffer):
    ...
    while True:
        ...
        if pid == 0:
            ...
            message += varint.to_stream(len(pkeystr)) + pkeystr
            message += varint.to_stream(len(verify_token)) + verify_token
            send_message(conn, string.to_stream(message))
        elif pid == 1:
            sec_len, startinc = varint.from_stream(pack[start:])
            start += startinc
            secret = pack[start:start+sec_len]
            start += sec_len
            tok_len, startinc = varint.from_stream(pack[start:])
            start += startinc
            ...
            if user is None:
                ...
                send_message(conn, string.to_stream("\x00"+string.to_stream(
                    "{'text':\"Failed to query Minecraft's Authentication"
                    " Server.\"}"
                )), cipher)
                conn.close()
                return
